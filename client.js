...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function buildQuoteBlock(entry, text) {
      var quoted = '> ' + text.replace(/\n+/g, '\n> ')
      if (entry === null) return quoted
      return t({
        zh: [
          '【MinerU 引用 · 文档：' + (entry.title || entry.id) + '】',
          '文档 ID：' + entry.id,
          'PDF 路径：' + (entry.pdf || '未知'),
          'HTML 文件：' + (entry.html || '未知'),
          'ZIP 文件：' + (entry.zip || '未知'),
          '预览链接：/mineru/preview/' + encodeURIComponent(entry.id),
          '',
          '引用内容：',
          quoted,
          '',
          '请结合这份 MinerU 解析文档来回答我的问题；如果需要，可读取上面的 HTML/PDF 路径或打开预览链接核对原文。',
        ].join('\n'),
        en: [
          '【MinerU quote · Document: ' + (entry.title || entry.id) + '】',
          'Document ID: ' + entry.id,
          'PDF path: ' + (entry.pdf || 'unknown'),
          'HTML file: ' + (entry.html || 'unknown'),
          'ZIP file: ' + (entry.zip || 'unknown'),
          'Preview link: /mineru/preview/' + encodeURIComponent(entry.id),
          '',
          'Quoted content:',
          quoted,
          '',
          'Please answer using this MinerU parsed document; you can read the HTML/PDF paths above or open the preview link to verify the source.',
        ].join('\n'),
      })
    }

    function quoteToDraft() {
      var text = ui.selectedText
      if (text === '') return
      try {
        if (sessions === null || conversation === null) {
          toast(t({ zh: '会话服务不可用', en: 'Conversation service unavailable' }))
          return
        }
        var current = sessions.list.getSnapshot().current
        if (current === undefined) { toast(t({ zh: '未找到当前会话', en: 'No current session' })); return }
        var scoped = sessions.scope(current)
        if (scoped === undefined) { toast(t({ zh: '会话作用域不可用', en: 'Session scope unavailable' })); return }
        var shell = conversation.input.for(scoped)
        var st = shell.state.getSnapshot()
        var draft = (st && st.draft) || ''
        var entry = ui.entries.find(function (e) { return e.id === ui.currentId }) || null
        var quoted = buildQuoteBlock(entry, text)
        var next = draft.trim() === '' ? quoted : draft.replace(/\s+$/, '') + '\n\n' + quoted
        shell.setDraft(next)
        toast(t({ zh: '已引用（含文档信息）到对话输入框', en: 'Quoted with document info into the conversation draft' }))
        hideQuote()
      } catch (err) {
    ...
    function buildModal() {
      ...
      quoteBtn.addEventListener('click', quoteToDraft)
