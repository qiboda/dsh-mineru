  ...
  factory: (require) => {
    ...
    if (document.getElementById(STYLE_ID) === null) {
      ...
      style.textContent = [
        ...
        '.dsh-mineru-settings-checkbox { flex-direction: row; align-items: center; gap: 6px; font-size: 12px; }',
        '.dsh-mineru-settings-checkbox input { flex: none; width: auto; }',
    ...
    function SettingsCheckboxField(props) {
      return React.createElement('label', { className: 'dsh-mineru-settings-checkbox' },
    ...
    function SettingsSection() {
      ...
      if (snap.loading) {
        ...
        children.push(
          ...
          React.createElement(SettingsCheckboxField, {
            key: 'field-auto-ocr',
            label: t({
              zh: '检测到日期数字缺失时自动切换 OCR 重新解析',
              en: 'Auto re-parse with OCR when date digits are missing',
            }),
            checked: (() => {
              const v = fieldValue('autoOcrFallback')
              return v === '' || v === 'true'
            })(),
            onChange: function (v) { patch('autoOcrFallback', v) },
          }),
    ...
    function loadScrollPos(id) {
      if (!id) return null
      try {
        var key = 'dsh-mineru-scroll:' + id
        var raw = localStorage.getItem(key)
        if (raw !== null && raw !== '') {
          var n = parseFloat(raw)
          if (!isNaN(n)) return n
        }
        // One-time migration: promote the current session's saved scroll
        // position to the shared global key so all DSH sessions start from
        // the same reading position.
        var sk = currentSessionKey()
        if (sk) {
          var perSession = localStorage.getItem('dsh-mineru-scroll:' + sk + ':' + id)
          if (perSession !== null && perSession !== '') {
            var m = parseFloat(perSession)
            if (!isNaN(m)) {
              localStorage.setItem(key, perSession)
              return m
            }
          }
        }
    ...
    function loadLastDocId() {
      try {
        var raw = localStorage.getItem('dsh-mineru-current-doc')
        if (raw !== null && raw !== '') return raw
        // One-time migration: promote the current session's last opened doc to
        // the shared global key so every DSH session sees the same history.
        var sk = currentSessionKey()
        if (sk) {
          var perSession = localStorage.getItem('dsh-mineru-current-doc:' + sk)
          if (perSession !== null && perSession !== '') {
            localStorage.setItem('dsh-mineru-current-doc', perSession)
            return perSession
          }
        }
        return null
    ...
    function applyThemeToFrame(frame, doc) {
      ...
      try {
        ...
        style.textContent = [
          ...
          'body { margin: 0; max-width: none !important; padding: 1.25rem 1.5rem; line-height: 1.7; }',
    ...
    function apply(core) {
      ...
      // React to DSH session switches so the shared last-opened MinerU
      // document / scroll position is refreshed even while the sidebar tab
      // stays mounted.
