...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function runReadingPositionMigration() {
      try {
        if (localStorage.getItem('dsh-mineru-migrate-reading-pos-v1') === '1') return
        var sk = currentSessionKey()
        if (sk) {
          // Promote the current session's per-session last-doc value.
          var perDoc = localStorage.getItem('dsh-mineru-current-doc:' + sk)
          if (perDoc !== null && perDoc !== '') {
          ...
          for (var i = 0; i < localStorage.length; i++) {
            ...
            if (raw !== null && raw !== '') {
              ...
              if (!isNaN(n) && isFinite(n)) {
                localStorage.setItem('dsh-mineru-scroll:' + id, String(Math.max(0, Math.round(n))))
              }
            }
          }
          localStorage.setItem('dsh-mineru-migrate-reading-pos-v1', '1')
        }
      } catch (_) { /* ignore */ }
    }
