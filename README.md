...
| Last position restore | The last opened document and its scroll position are remembered in `localStorage` (`dsh-mineru-current-doc`, `dsh-mineru-scroll:<id>`). Re-opening the MinerU panel/tab automatically loads the last document and restores the previous reading position. |
