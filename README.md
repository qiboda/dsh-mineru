...
| Last position restore | The last opened document and its scroll position are shared **across all DSH sessions** in `localStorage` (`dsh-mineru-current-doc`, `dsh-mineru-scroll:<id>`). Re-opening the MinerU panel/tab, or switching DSH conversations while the tab stays open, loads the shared last document and restores the shared reading position. Previous per-session keys are migrated once with `dsh-mineru-migrate-reading-pos-v1`. |
