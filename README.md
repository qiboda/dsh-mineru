...
| Full-width HTML preview | Pandoc's default 36em body cap is removed when generating the HTML, and older previews get the same full-width override when served, so text follows the resizable preview width. |
| Last position restore | The last opened document and its scroll position are shared **across all DSH sessions** in `localStorage` (`dsh-mineru-current-doc`, `dsh-mineru-scroll:<id>`). Re-opening the MinerU panel/tab, or switching DSH conversations while the tab stays open, loads the shared last document and restores the shared reading position. The active session's previous per-session keys are migrated once with `dsh-mineru-migrate-reading-pos-v1`. |
...
| Date-loss OCR repair | Some Chinese PDFs lose inline date digits through MinerU's default text layer (e.g. `年 月 日`, `年3 月 1 日`). The host detects these gaps in the generated Markdown and automatically re-parses with `method: ocr` (hybrid backend, high effort) before converting to HTML. Disable or keep it on with `autoOcrFallback` in the MinerU settings or `DSH_MINERU_AUTO_OCR_FALLBACK=0` / `false`. |
...
- To change defaults: use the DSH Settings page → MinerU, or the environment
  variables `DSH_MINERU_LIBRARY`, `DSH_MINERU_INDEX`, `MINERU_BIN`, and
  `DSH_MINERU_AUTO_OCR_FALLBACK` (`0`/`false` disables the date-loss OCR repair).
